...
import { FHEVM_AWAIT_DECRYPTION_ORACLE, FHEVM_GET_CLEAR_TEXT, RELAYER_METADATA, RELAYER_V1_INPUT_PROOF, RELAYER_V1_PUBLIC_DECRYPT, RELAYER_V1_USER_DECRYPT, } from "./methods.js";
import { assertIsMockRelayerV1InputProofPayload } from "./mock_payloads.js";
import { assertIsRelayerMetadata, assertIsRelayerV1InputProofResponse, assertIsRelayerV1PublicDecryptPayload, assertIsRelayerV1UserDecryptPayload, } from "./payloads.js";
/**
 * Equivalent to const response = await fetch(`${relayerUrl}/v1/input-proof`, options);
 */
export async function requestRelayerV1InputProof(provider, payload) {
 ...
 */
export async function requestRelayerV1UserDecrypt(provider, payload) {
 ...
 */
export async function requestRelayerV1PublicDecrypt(provider, payload) {
 ...
 */
export async function requestRelayerMetadata(provider) {
    const response = await minimalProviderSend(provider, RELAYER_METADATA, []);
    assertIsRelayerMetadata(response);
    return response;
}
export async function requestFhevmAwaitDecryptionOracle(provider) {
    return await minimalProviderSend(provider, FHEVM_AWAIT_DECRYPTION_ORACLE, []);
}
export async function requestFhevmGetClearText(provider, payload) {
