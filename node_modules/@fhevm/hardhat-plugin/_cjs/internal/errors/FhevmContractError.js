...
async function mutateErrorInPlace(fhevmEnv, e, args) {
    ...
    Object.keys(map).forEach((key) => {
        err.stack = err.stack?.replaceAll(`at <UnrecognizedContract>.<unknown> (${key})`, `at ${map[key].contractName}.<unknown> (${key}, ${map[key].package}/contracts/${map[key].contractName}.sol:0:0)`);
    });
}
...
function _formatFHEGasLimitErrorMessages(infos) {
    return _applyErrorTemplates(infos);
}
...
function _applyErrorTemplates(infos, values) {
    ...
    if (infos.contractName in FhevmContractErrorList_1.ERRORS) {
        const error = FhevmContractErrorList_1.ERRORS[infos.contractName];
