...
exports.getRelayerSignerIndex = getRelayerSignerIndex;
...
exports.envGetHardhatSignersAddresses = envGetHardhatSignersAddresses;
...
exports.getFHEGasLimitAddress = getFHEGasLimitAddress;
...
function getRelayerSignerIndex() {
    try {
...
async function getCoprocessorSigners(hre, provider) {
    try {
        const hhSigners = await hre.ethers.getSigners();
        const coprocessorSignersAddresses = envGetHardhatSignersAddresses("NUM_COPROCESSORS", "COPROCESSOR_SIGNER_ADDRESS_", hhSigners);
...
async function getKMSSigners(hre, provider) {
    try {
        const hhSigners = await hre.ethers.getSigners();
        ...
        const signer = new ethers_1.ethers.Wallet(kmsSignerKey).connect(provider);
        return [signer];
    }
}
...
function envGetHardhatSignersAddresses(numEnvVarName, listEnvVarNamePrefix, hhSigners) {
...
function getFHEGasLimitAddress() {
    const address = __getStringConstant("FHE_GASLIMIT_CONTRACT_ADDRESS", constants_1.default["FHE_GASLIMIT_CONTRACT_ADDRESS"], "addresses/.env.fhegaslimit");
    if (!ethers_1.ethers.isAddress(address)) {
        throw new error_1.HardhatFhevmError(`Invalid FHEGasLimit address ${address}`);
    }
    return address;
}
