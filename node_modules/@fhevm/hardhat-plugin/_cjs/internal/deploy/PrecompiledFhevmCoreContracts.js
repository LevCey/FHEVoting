...
function __logPrecompiledAddresses(addresses, useCache) {
    ...
    debug(`${prefix} precompiled ${picocolors.cyanBright("FHEGasLimitAddress")} address   : ${addresses.FHEGasLimitAddress}`);
}
...
async function getPrecompiledFhevmCoreContractsAddresses(ethersProvider, fhevmPaths) {
    ...
    if (ethersProvider.info.networkName !== "hardhat") {
        throw new error_1.HardhatFhevmError(`Can't retrieve ${constants_1.default.FHEVM_CORE_CONTRACTS_PACKAGE_NAME} precompiled addresses. This operation is only supported on 'hardhat' network. Current network: ${ethersProvider.info.networkName}`);
    }
    const precompiledFHEVMExecutorAddress = await retrievePreCompiledFHEVMExecutorAddressFromACLArtifact(ethersProvider, fhevmPaths);
    try {
        ...
        const FHEVMExecutorReadOnly = new ethers_1.ethers.Contract(precompiledFHEVMExecutorAddress, FHEVMExecutorArtifact.artifact.abi, ethersProvider.provider);
...
async function retrievePreCompiledFHEVMExecutorAddressFromACLArtifact(ethersProvider, fhevmPaths) {
    ...
    try {
        ...
        const dummyAcl = new ethers_1.ethers.Contract(DUMMY_ACL_ADDR, aclArtifact.artifact.abi, ethersProvider.provider);
...
async function loadPrecompiledFhevmCoreContractsAddresses(ethersProvider, fhevmPaths, ignoreCache, isRunningInHHFHEVMInstallSolidity) {
